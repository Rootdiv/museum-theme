...
function museum_theme_comment( $comment, $args, $depth ) {
      ...
      <div class="comment-content">
        <div class="comment-author vcard">
          ...
          <span class="comment-meta commentmetadata">
            ...
            <?php edit_comment_link( __( '(Edit)', 'museum' ), '  ', '' ); ?>
          </span>
        </div>

        <?php if ( $comment->comment_approved == '0' ) { ?>
          <em class="comment-awaiting-moderation">
            <?php _e( 'Your comment is awaiting moderation.', 'museum' ); ?>
          </em><br/>
        <?php } ?>
  ...
  <div id="comments" class="comments-area">
    ...
    if ( have_comments() ) :
      ...
      <div class="comments-header">
        <h2 class="comments-title">
          <?php echo __('Comments', 'museum').' <span class="comments-count">'. get_comments_number() .'</span> ';?>
        </h2><!-- .comments-title -->
      ...
      <ol class="comments-list">
        ...
        wp_list_comments(
          array(
            ...
            'short_ping' => true,
            'avatar_size'=> 75,
            'callback' => 'museum_theme_comment',
            'login_text' => __('Register if you want to comment', 'museum'),
          )
        );
        ?>
      </ol><!-- .comments-list -->
    ...
    comment_form(array(
      'comment_field'         => '<div class="comment-form-comment">
        <label class="comment-label" for="comment">'._x( 'What do you think?', 'noun', 'museum' ) . '</label>
      ...
      'must_log_in'           => '<p class="must-log-in">'.
        sprintf( __('You must be <a href="%s">logged in</a> to post a comment.', 'museum'), 
        wp_login_url( apply_filters('the_permalink', get_permalink()))) .'
      </p>',
      'logged_in_as'          => '',
      'comment_notes_before'  => '<p class="comment-notes">
        <span id="email-notes">' . __( 'Your email address will not be published.', 'museum' ) . '</span>'. 
        ( $req ? $required_text : '' ) . '
      </p>',
      'class_submit'          => 'comment-submit more',
      'label_submit'          => __('Send', 'museum'),
